...
public class QnaServiceImpl implements QnaService {
	...
	public List<TopicModel> saveTopicDetails(TopicModel user) {
		if (getTopicId(user.getTopicNumber()) == 0) {
			topicRepository.save(user);
			return getFullTopicForUser();
		}
		return null;
	}
	...
	public boolean saveQuestions(TopicModel user) throws QnaException {
		try {
			...
			int topicId = getTopicId(user.getTopicNumber());
			if (topicId != 0) {
	...
	public List<QuestionModel> getQuestions(TopicModel user) throws QnaException {
		try {
			...
			int topicId = getTopicIdFromTopicNumber(user);
	...
	private List<TopicModel> getFullTopicForUser() {
		return topicRepository.findAll();
	}

	private int getTopicId(int topicNumber) {
	...
	private int getTopicIdFromTopicNumber(TopicModel user) {
		List<TopicModel> topics = getFullTopicForUser();
		for (TopicModel topic : topics) {
			if (topic.getTopicNumber() == user.getTopicId()) {
				return topic.getTopicId();
			}
		}
		return 0;
